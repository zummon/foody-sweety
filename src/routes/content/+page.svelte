...
<div class="flex flex-wrap justify-center gap-4 max-w-lg mx-auto text-lg font-medium">
	{#each data.tags as tag, index (`tag-${index}`)}
		<button class="py-0.5 px-1 sm:px-2 rounded-full transition duration-300 cursor-pointer {tags.includes(tag) ? 'text-orange-600 bg-orange-100 dark:text-orange-400 dark:bg-orange-900' : 'text-zinc-100 bg-zinc-600 dark:text-zinc-900 dark:bg-zinc-400'}" onclick={() => {
			if (tags.includes(tag)) {
...
{#each foods as food, index (index)}
	<div
		class="dark:bg-zinc-800 bg-zinc-50 max-w-2xl rounded-xl mx-auto my-8 shadow-lg shadow-yellow-200 dark:shadow-yellow-950 sm:flex flex-wrap"
	>
		<div class="flex-1">
			<img
